...
bool ControllerBuffer::allocate(uint32_t num_agents) {
    ...
    const size_t total_size = header_size + agent_slots_size + result_size + cmd_area_size;
    ...
    result_offset_ = static_cast<uint32_t>(header_size + agent_slots_size);
    cmd_area_offset_ = static_cast<uint32_t>(header_size + agent_slots_size + result_size);
